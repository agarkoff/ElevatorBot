...
ElevatorBot - это Telegram бот для удаленного управления лифтом в многоквартирном доме. Бот позволяет авторизованным пользователям вызывать лифт на определенный этаж через мобильное приложение Telegram.

## Основные возможности

- 📱 Авторизация пользователей по номеру телефона
- 🔒 Проверка доступа через белый список номеров
- 🏢 Вызов лифта на выбранный этаж
...
- Доступ к backend-сервису управления лифтом
...
### 6. Конфигурация этажей

Создайте файл `floors.properties` в каталоге с JAR-файлом:

```properties
# Файл конфигурации этажей
# Формат: floor=relay
# floor - номер этажа, который видит пользователь
# relay - номер реле для управления

8=0
13=1

# Можно добавить дополнительные этажи
# 1=2
# 2=3
# 3=4
```

#### Особенности конфигурации этажей:
...
- **Сортировка**: этажи отображаются в порядке возрастания
...
├── floors.properties                           # Конфигурация этажей
...
4. **Выбор этажа**: После успешной авторизации появляется клавиатура с этажами
5. **Вызов лифта**: При выборе этажа отправляется команда на backend-сервис
...
- "Выберите этаж" - успешная авторизация
- "Команда отправлена" - лифт успешно вызван
- "Слишком часто, ждите X секунд" - защита от частых вызовов
- "Команда не отправлена" - ошибка при вызове лифта
...
- `relay` - номер реле (из конфигурации этажей)
...
- Проверьте файл `floors.properties`
...
COPY floors.properties floors.properties
